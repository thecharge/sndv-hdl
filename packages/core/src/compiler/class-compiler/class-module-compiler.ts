...
export class ClassModuleParser {
  ...
  private parseAssignOrExpr(): StatementAST {
    ...
    // (e.g. `highTicks = this.t1h;` inside a block where highTicks was declared with `let`)
...
export class ClassModuleEmitter {
  ...
  private emitSequential(method: MethodAST, mod: ClassModuleAST, enums: EnumAST[], pw: Map<string, number>): void {
    ...
    // Collect all let/const declarations from inside the method body and emit
    // them as module-level logic registers (synthesizable; automatic vars unsupported by Yosys).
    ...
    if (localVars.length > 0) {
      ...
      for (const v of localVars) {
        ...
        const w = formatWidth(width);
        this.line(`    logic ${w} ${vname};`);
        // Add to property-width map so assignments to this local get sized correctly
        pw.set(v.name, width);
      }
      this.line('');
    }
  ...
  private collectVarDecls(stmts: StatementAST[]): VarDeclAST[] {
  ...
  private resolveLocalVarWidth(varDecl: VarDeclAST, pw: Map<string, number>): number {
    // 1. Explicit type annotation: Logic<N>, UintN, Bit, boolean
    if (varDecl.type) {
    ...
    // 2. Infer from RHS if it is a plain this.property reference
    const propRef = varDecl.value.match(/^this\.(\w+)$/);
    if (propRef) {
      const w = pw.get(propRef[1]);
      if (w !== undefined) return w;
    }
    // 3. Result of & 1 is always 1 bit
  ...
  private emitStatement(stmt: StatementAST, enums: EnumAST[], mod: ClassModuleAST, is_seq: boolean, pw: Map<string, number>): void {
    ...
    switch (stmt.kind) {
      ...
      case 'var': {
        // In sequential context the declaration was already emitted before always_ff.
        // In combinational context (always_comb) emit as blocking assignment with declaration.
  ...
  private sizeLiteral(expr: string, width: number): string {
    ...
    // Already-sized SV literal (simple form): "8'h3f", "32'd0", "4'b0101"
    // Resize to target width while keeping base and value.
    const svLitM = expr.match(/^(\d+)'([bBhHdD])([0-9a-fA-F_xzXZ]+)$/);
    if (svLitM) {
      return `${width}'${svLitM[2].toLowerCase()}${svLitM[3]}`;
    }
    // Anything else that already has a size specifier → leave as-is
